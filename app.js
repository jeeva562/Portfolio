...
const getOpenAIKey = () => {
    if (typeof config !== 'undefined' && config.getOpenAIKey) {
...
const getHuggingFaceKey = () => {
    if (typeof config !== 'undefined' && config.getHuggingFaceKey) {
        return config.getHuggingFaceKey();
    }
    return prompt('Please enter your Hugging Face API key:');
};
...
const handleSendPrompt = async () => {
    console.log('handleSendPrompt triggered'); // Debug log
    ...
    if (!prompt && !imageFile) {
        console.log('No prompt and no image, exiting');
        return;
    }

    // Ensure elements exist
    if (!resultBox || !resultContent || !loadingIndicator || !sendButton) {
    ...
    try {
        ...
        console.log('Is image generation:', isImageGen);
...
const displayError = (errorMessage) => {
    ...
    if (isCORSError) {
        ...
        troubleshootingTips += `
                    <li>Check if your API keys are valid</li>
    ...
    troubleshootingTips += `
                    <li>Consider upgrading to Pro for better reliability</li>
                </ul>
            </div>
        </div>
    `;
...
const handleTextGeneration = async (prompt) => {
    console.log('handleTextGeneration called with prompt:', prompt);
...
if (promptInput) {
    ...
    console.log('Prompt input event listener added');
}

if (fileInput) {
    fileInput.addEventListener('change', (event) => {
        const file = event.target.files[0];
        if (file && imagePreviewImg && imagePreview) {
            const reader = new FileReader();
            reader.onload = (e) => {
